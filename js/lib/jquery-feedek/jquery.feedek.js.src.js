...
(function ($) {
    $.fn.FeedEk = function (opt) {
        ...
        var id = $(this).attr("id");
        var i;
        $("#" + id).empty().append('<img src="img/activity_indicator_white.gif" />');
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.responseData.feed.entries, function (e, item) {
                    s += '<li class="list-group-item"><div class="itemTitle"><a href="' + item.link + '" target="' + def.TitleLinkTarget + '" >' + item.title + "</a></div>";
                    if (def.ShowPubDate) {
                ...
                $("#" + id).append('<ul class="feedEkList list-group">' + s + "</ul>");
            }
        });
    };
})(jQuery);
