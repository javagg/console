...
define([
    ...
    'util/button-helper',
    'util/role-count',
    'text!views/settings/rabbitmq/templates/settings.html',
    ...
    function ($, Backbone, _, Polyglot, Async, Gravatar, Activity, MaintenanceMode, ButtonHelper, RoleCount, SettingsTemplate, AlertTemplate) {
        ...
        return Backbone.View.extend({
            ...
            initialize: function () {
                ...
                RoleCount.renderNodeCount('rabbit', '.role-count-container');
            },
            ...
            getRabbitMQConfig: function () {
                ...
                sconsole.cf_api.config.getRabbitNodeConfig({}, function (err, rabbit_node_config) {
                    node_activity.close();
                    if (!err) {
                ...
                sconsole.cf_api.config.getRabbitGatewayConfig({}, function (err, rabbit_gateway_config) {
            ...
            saveRabbitMQSettings: function (event) {
                ...
                Async.parallel({
                    capacity: function (done) {
                        sconsole.cf_api.config.updateRabbitNodeConfigValue('capacity', capacity, {}, done);
                    },
                    max_memory: function (done) {
                        sconsole.cf_api.config.updateRabbitNodeConfigValue('max_memory', max_memory, {}, done);
                    },
                    overprovisioning: function (done) {
                        sconsole.cf_api.config.updateRabbitGatewayConfigValue(
                            'plan_management',
                            {
                                plans: {
                                    free: {
